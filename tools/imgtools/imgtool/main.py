...
@click.option('--payload-crypted', default=False, is_flag=True,
              required=False,
              help='payload is pre-proceeded by AES256 of Verisilicon')
...
@click.option('-k', '--key', metavar='filename')
@click.option('--bl-img', help='image is bootloader or not')
@click.option('--entry', help='Section contains entry point or not')
@click.option('--ram-load', help='Section image is loaded to RAM or not')
@click.command(help='''Create a signed or unsigned image\n
               INFILE and OUTFILE are parsed as Intel HEX if the params have
               .hex extension, otherwise binary format is used''')
def sign(key, public_key_format, align, version, pad_sig, header_size,
         pad_header, slot_size, pad, payload_crypted, confirm, max_sectors, overwrite_only,
         endian, encrypt, infile, outfile, dependencies, load_addr, hex_addr,
         erased_val, save_enctlv, security_counter, boot_record, bl_img, entry, ram_load):
    img = image.Image(version=decode_version(version), header_size=header_size,
                      pad_header=pad_header, pad=pad, payload_crypted=payload_crypted,confirm=confirm,
    ...
    img.create(key, public_key_format, enckey, bl_img, entry, ram_load, dependencies, boot_record)
