...
IMAGE_F = {
        ...
        'IMG_IS_BOOTLOADER':     0x80000000,
        'ENTRY_CONTAINED':       0x40000000,
        'LOAD_TO_RAM':           0x20000000,
}
...
class Image():
    ...
    def __init__(self, version=None, header_size=IMAGE_HEADER_SIZE,
                 pad_header=False, pad=False, payload_crypted=False, confirm=False, align=1,
        ...
        self.pad = pad
        self.payload_crypted = payload_crypted
    ...
    def create(self, key, public_key_format, enckey, bl_img, entry, ram_load, dependencies=None,
               sw_type=None):
        self.enckey = enckey
        ...
        self.add_header(enckey, protected_tlv_size, bl_img, entry, ram_load)
    ...
    def add_header(self, enckey, protected_tlv_size, bl_img, entry, ram_load):
        """Install the image header."""

        flags = 0
        if enckey is not None:
            flags |= IMAGE_F['ENCRYPTED']
        if self.payload_crypted is True:
            flags |= IMAGE_F['ENCRYPTED']
        if self.load_addr != 0:
        ...
        if bl_img == "True":
            flags |= IMAGE_F['IMG_IS_BOOTLOADER']
        if entry == "True":
            flags |= IMAGE_F['ENTRY_CONTAINED']
        if ram_load == "True":
            flags |= IMAGE_F['LOAD_TO_RAM']
